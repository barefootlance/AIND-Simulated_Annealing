# NOTE: I don't know that Jupyter notebooks are really as useless as they seem to me,
# but they sure as hell are frustrating. So I copied the code here and worke through it.
# It was a much nicer eperience doing that.
...
def simulated_annealing(problem, schedule):
    ...
    import random
    import math
    close_enough = math.exp(-10)
    current = None
    t = 0
    while True:
        t += 1 # NOTE: no sys.maxint in py3, so can't use range in a for loop
        temp = schedule(t)
        if temp < close_enough:
            return current
        nxt = random.choice(problem.successors())
        delta = nxt.get_value() - problem.get_value()
...
class TravelingSalesmanProblem:
    ...
    def successors(self):
        ...
        for i in range(0, len(self.path)):
            n = self.copy() # make a copy of the current state
            n.path[i-1], n.path[i] = n.path[i], n.path[i-1] # safe swap in python
            neighbors.append(n) # add to the list of neigbhors
        return neighbors
    ...
    assert(tsp.path == test_cities)
        ...
        """
        pts = self.coords
        deltas = [((float(pts[i-1][0] - pts[i][0]))**2 + (float(pts[i-1][1] - pts[i][1]))**2)**0.5 for i in range(0, len(pts))]
        return -sum(deltas)

def test_makeProblem():
...
def test_validate_successors_and_get_value():

    tsp = test_makeProblem()
...
import math
def schedule(time):
    return math.pow(alpha, time) * temperature
...
#test_validate_successors_and_get_value()
#test_temp_schedule()
...
print("Improvement: {:.2f}".format(1.0 - (result.get_value()/capitals_tsp.get_value())))